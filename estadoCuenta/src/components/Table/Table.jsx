        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide"># Factura</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide">Cuotas</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide">
                  Fecha máxima
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide">
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide">Días</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wide">Estado</th>
              </tr>
            </thead>
                  ...
                  clienteData.facturas.forEach((factura) => {
                    clientRows.push(
                      <tr key={`factura-${factura.id}`} className="bg-gray-100 hover:bg-red-100 transition-colors">
                        <td className="px-6 py-3 text-xs font-bold text-gray-600">{factura.numero}</td>
                        <td className="px-6 py-3 text-xs text-gray-700">{factura.fecha}</td>
                    ...
                    factura.cuotas.forEach((cuota, index) => {
                      ...
                      clientRows.push(
                        <tr key={`cuota-${factura.id}-${index}`} className="hover:bg-red-50 transition-colors group">
                          ...
                          <td className="px-6 py-3 text-sm text-gray-600"></td>
                          {/* <td className="px-6 py-3 text-sm text-gray-700">{cuota.descripcion}</td> */}
                          <td className="px-6 py-3 text-xs text-gray-700">Cuota {index + 1}</td>
